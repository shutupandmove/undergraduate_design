...
Q = diag([1,10,1,50,50,0]);%n*n
F = diag([1,10,1,50,50,0]);%n*n
%     [u_r,u_t,u_n]
R = diag([1,1,1]);%p*p
...
% 距离上选取10m作为极限
% 角度上选取1e-3deg≈1.7453e-05rads作为极限
%% 绘制状态变量和输入的变化
...
legend("da");
hold off;
title("Q = diag([1,10,1,50,50,0]) F = diag([1,10,1,50,50,0]) R = diag([1,1,1])");
% title("无校正");
